	...
	public static function Load(GenericEvent $event) {
		...
		$service->set_module('vps')
			->set_enable(function($service) {
				...
				$smarty = new \TFSmarty;
				$smarty->assign('vps_name', $serviceTypes[$service_info[$settings['PREFIX'] . '_type']]['services_name']);
				$email = $smarty->fetch('email/admin_email_vps_reactivated.tpl');
				$subject = $service_info[$settings['TITLE_FIELD']].' '.$serviceTypes[$service_info[$settings['PREFIX'] . '_type']]['services_name'].' '.$settings['TBLNAME'].' Re-Activated';
				$headers = '';
